...
export default class ShowReferralUtils {
  ...
  static statusHistoryTimelineItems(
    ...
    previousReferralHref?: string,
    previousReferralName?: string,
    ...
    return statusHistoryPresenter.map((status, index) => {
      ...
      if (previousReferralHref && index === statusHistoryPresenter.length - 1) {
        html = html.concat(
          `<hr class="govuk-section-break govuk-section-break--m govuk-section-break--visible">This referral was transferred from ${previousReferralName}. See <a href="${previousReferralHref}">previous referral details</a>`,
        )
      }
